...
% 在这个程序里面 x(1)x(2)表示关节1的位置速度，x(2)x(4)表示关节1的位置速度
%% desired trajectory
...
e=[x(1) x(3)]-yd; e=e'; %保证e是列向量
de=[x(2) x(4)]-dyd; de=de';
...
m11=(m1+m2)*l1^2+m2*l2^2+2*m2*l1*l2*cos(x(3));
m12=m2*l2^2+m2*l1*l2*cos(x(3));
m21=m12;
m22=m2*l2^2;
M = [m11 m12; m21 m22];   
%% computation of C matrix
c11=-m1*l1*l2*x(4)*sin(x(3));
c12=-m2*l1*l2*(x(2)+x(4))*sin(x(3));
c21=m2*l1*l2*x(2)*sin(x(3));
c22=0;
C=[c11 c12; c21 c22]*[x(2);x(4)];
%C1=[c11 c12; c21 c22];
 %% computation of G matrix
g1=m1*l1*9.8*cos(x(1))+m2*9.8*(l2*cos(x(1)+x(3))+l1*cos(x(1)));
g2=m2*l2*9.8*cos(x(1)+x(3));
G = [g1;g2];
...
xdot=[x(2);
      ...
      x(4); 
      ydotdot(2); %% 输出关节2向量
      (u(1)-x(5));%%ode will not integrate this, output only
      (u(2)-x(6));%%ode will not integrate this, output only
